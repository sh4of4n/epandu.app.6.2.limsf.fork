...
*/
  ...
  Future<List<Room>> getRoomList(String userId) async {
    Database db = await instance.database;
    var res = await db.rawQuery(
        "Select $roomMembersTable.user_id,$roomTable.create_date, $roomTable.room_id,$roomTable.picture_path,$roomTable.room_name,$roomTable.room_desc from $roomTable LEFT JOIN $roomMembersTable ON $roomMembersTable.room_id=$roomTable.room_id  where $roomMembersTable.user_id = '$userId';");
    List<Room> list =
        res.isNotEmpty ? res.map((m) => Room.fromJson(m)).toList() : [];
    return list;
  }
  ...
  Future<int> updateRoomMemberPic(String? userId, String picturePath,
    ...
    Database db = await instance.database;
    return await db.rawUpdate(
        "UPDATE $roomMembersTable SET picture_path = ?,nick_name = ?,deleted = ? where user_id = ?",
        [
          picturePath,
          nickName,
          deleted,
  ...
  Future<int> deleteRoomMembers() async {
    Database db = await instance.database;
    return await db.delete(roomMembersTable);
  }
